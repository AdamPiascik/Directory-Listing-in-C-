...
namespace DirectoryMethods
{
    ...
    class ContentList
    {
        ...
        /*  This method populates the fileList and folderList lists. It uses
            ...
            and subfolders in the directory. If the folder can't be accessed,
            ...
            if (listType == "folders"){
                foreach (DirectoryInfo folder in this.dir.GetDirectories()){
                    try{
                        ...
                        Console.WriteLine(ex.Message + " Skipping folder...");
                    }
                }
            }
        ...
        /*  This method calculates the size of a folder by summing the sizes
            of all files in the folder and constituent subfolders. If the folder
            can't be accessed, an error message is written to the console. */
        long getFolderSize(DirectoryInfo thisFolder)
        {
            long thisFolderSize = 0;
            try{
                FileInfo[] folderFiles = thisFolder.GetFiles();
                foreach (FileInfo file in folderFiles)
                    thisFolderSize += file.Length;            
                foreach(DirectoryInfo subfolder in thisFolder.GetDirectories())
                 thisFolderSize += getFolderSize(subfolder);
            }
            catch (UnauthorizedAccessException ex){
                Console.WriteLine(ex.Message + " Skipping folder...");
            }
        ...
        /*  This method divides long file/folder names into chunks of length at
            most that specified by fieldWidth. This is to help readability in
